...
#define execve_or_die(filename, argv, envp) assert(execve(filename, argv, envp) == 0);
...
#define chdir_or_die(path) assert(chdir(path) == 0);
...
#define close_or_die(fd) assert(close(fd) == 0);
...
#define stat_or_die(filename, buf) assert(stat(filename, buf) >= 0);
...
#define munmap_or_die(start, length) assert(munmap(start, length) >= 0);
...
#define readline_or_die(fd, buf, maxlen)                                                           \
  ({                                                                                               \
    ssize_t rc = readline(fd, buf, maxlen);                                                        \
    assert(rc >= 0);                                                                               \
    rc;                                                                                            \
  })
#define open_client_fd_or_die(hostname, port)                                                      \
  ({                                                                                               \
    int rc = open_client_fd(hostname, port);                                                       \
    assert(rc >= 0);                                                                               \
    rc;                                                                                            \
  })
